...
## 📊 プロジェクト概要
学生の試験成績データをもとに、成績上位者の特徴を分析しました。  
SQLによるデータ抽出と、Pythonによる可視化を組み合わせて、成績に影響する要因を特定しています。
...
- データ不整合を検出・修正した経験　　　　　　　→　Sleep_HoursやPrevious_Scoresなどの値がすべて0になる不整合が発生した。
- 　　　　　　　　　　　　　　　　　　　　　　　　　そこで、
- 　　　　　　　　　　　　　　　　　　　　　　　　　df = pd.read_sql("SELECT * FROM student_performance", engine)
- 　　　　　　　　　　　　　　　　　　　　　　　　　を書き、テーブル定義とCSVの列順を一致させて再取り込みを行った。
...
- ダッシュボード化（Plotly / BIツール）　→まだまだこれから、段階的に進めていく。
